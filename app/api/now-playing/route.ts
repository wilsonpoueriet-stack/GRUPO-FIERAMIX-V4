...
function splitTrack(value: string) {
  ...
  if (separator < 0) return { artist: "", title: value.trim() };
  return {
...
export async function GET(request: NextRequest) {
  ...
  try {
    const endpoint = `${config.apiBase}/api/info/${encodeURIComponent(config.stationId)}?key=${encodeURIComponent(radioBossApi.apiKey)}`;
